(function () {
	...
	function isMacApp (folderPath) {
	  try {
	    const stats = lstatSync$2(folderPath);

	    if (!stats.isDirectory()) {
	      return false; // Not a directory, so not an app bundle
	    }

	    const contentsPath = path$4.join(folderPath, 'Contents');
	    const macOSPath = path$4.join(contentsPath, 'MacOS');
	    const infoPlistPath = path$4.join(contentsPath, 'Info.plist');

	    // Check for the presence of key directories and files
	    const contentsExists = lstatSync$2(contentsPath).isDirectory();
	    const macOSExists = lstatSync$2(macOSPath).isDirectory();
	    const infoPlistExists = lstatSync$2(infoPlistPath).isFile();
